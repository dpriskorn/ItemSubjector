...
#                 f"""
#             #{config.user_agent}
#             SELECT DISTINCT ?main_subject_item ?itemLabel
#             WHERE {{
#               hint:Query hint:optimizer "None".
#               SERVICE wikibase:mwapi {{
#                 bd:serviceParam wikibase:api "Search";
#                                 wikibase:endpoint "www.wikidata.org";
#                                 mwapi:srsearch 'haswbstatement:P8433 -haswbstatement:P921={suggestion.main_subject_item.id} "{search_string}"' .
#                 ?title wikibase:apiOutput mwapi:title.
#               }}
#               BIND(IRI(CONCAT(STR(wd:), ?title)) AS ?main_subject_item)
...
#               MINUS {{?main_subject_item wdt:P921 ?topic. ?topic wdt:P279 wd:{suggestion.main_subject_item.id}. }}
